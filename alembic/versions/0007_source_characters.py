...
recreates the schema once when it fingerprints the legacy pre-squash schema (the
old `manuscripts` table the new chain replaced with `sources`; revision ids were
reused so the stamp is unreliable), so there is no manual step and `characters` is
empty when this runs (a backfill would be a no-op). Existing casts are (re)linked
when their sources are re-extracted and committed. If a future change must preserve
live data, disable the auto-reset (`POLYPHONY_FORCE_SCHEMA_RESET=0`) and add:
