...
from services.shared.database import check_db_connection
...
from routers import auth, manuscripts, scenes  # noqa: E402
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info(
        "Starting Polyphony API Gateway", extra_fields={"event": "service_startup"}
    )
    ...
    # Shutdown
    logger.info(
        "Shutting down Polyphony API Gateway",
...
async def log_requests(request: Request, call_next):
    ...
    if request_size > 0:
        http_request_size_bytes.labels(
            method=request.method, endpoint=request.url.path, service="api-gateway"
    ...
    log_request_end(
        logger, request.method, request.url.path, response.status_code, duration_ms
    )
