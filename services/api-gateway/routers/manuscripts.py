...
async def process_manuscript_background(
    manuscript_id: UUID, file_id: str, user_id: UUID
):
    """
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            print(
                f"Extracted {len(characters)} characters from manuscript {manuscript_id}"
            )
    ...
    try:
        ...
        if background_tasks:
            background_tasks.add_task(
                process_manuscript_background, manuscript.id, file_id, current_user.id
            )
...
async def get_manuscript(
    ...
    result = await db.execute(
        select(ManuscriptORM).where(
            ManuscriptORM.id == manuscript_id, ManuscriptORM.user_id == current_user.id
        )
    )
